...
export async function updateProfile(req, res, next) {
  try {
    ...
    const patient = await Patient.findOneAndUpdate(
      { userId: req.user.userId }, { $set: update }, { new: true }
    );
    if (req.body.language) {
      await User.findByIdAndUpdate(req.user.userId, { language: req.body.language });
    }
    res.json({ success: true, data: { ...patient.toObject(), language: req.body.language ?? undefined } });
  } catch (err) { next(err); }
}
...
export async function upsertRecord(req, res, next) {
  try {
    ...
    if (!record?.date) return res.status(400).json({ success: false, message: 'date required' });
    const patient = await Patient.findOne({ userId: req.user.userId });
    if (!patient) return res.status(404).json({ success: false, message: 'Patient not found' });
    const idx = patient.records.findIndex(r => r.date === record.date);
    if (idx >= 0) patient.records[idx] = { ...patient.records[idx].toObject(), ...record };
...
export async function bulkUpsertRecords(req, res, next) {
  try {
    ...
    for (const rec of records) {
      ...
      if (idx >= 0) patient.records[idx] = { ...patient.records[idx].toObject(), ...rec };
...
export async function updateQuestionnaire(req, res, next) {
  try {
    const allowed = ['latestGad7', 'latestPhq9', 'latestAudit', 'latestDast10', 'latestCage', 'latestReadiness'];
    const { key, data } = req.body;
    if (!allowed.includes(key))
      return res.status(400).json({ success: false, message: `key must be one of ${allowed.join(', ')}` });
    const patient = await Patient.findOneAndUpdate(
      { userId: req.user.userId },
      { $set: { [key]: { ...data, date: new Date().toISOString().slice(0, 10) } } },
      { new: true }
    );
    res.json({ success: true, data: patient });
  } catch (err) { next(err); }
...
export async function updateMedicines(req, res, next) {
  try {
    ...
    const patient = await Patient.findOneAndUpdate(
      { userId: req.user.userId }, { $set: { medicines } }, { new: true }
    );
    res.json({ success: true, data: patient });
  } catch (err) { next(err); }
}
...
export async function createMedication(req, res, next) {
  try {
    ...
    const med = await Medication.create({
      userId: req.user.userId, name, dosage, frequency, startDate, prescribedBy, notes,
    });
...
export async function deleteMedication(req, res, next) {
  try {
    await Medication.findOneAndDelete({ _id: req.params.id, userId: req.user.userId });
...
export async function bulkSaveMedications(req, res, next) {
  try {
    ...
    const patient = await Patient.findOneAndUpdate(
      { userId: req.user.userId },
      { $set: { medicines: medications.map(id => ({ id: String(id), name: String(id) })) } },
...
// Share logic moved to shareController.js
